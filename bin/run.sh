...
docker run -d -it --cap-add=SYS_ADMIN --cap-add=NET_ADMIN --net=host --tmpfs /run -v /sys/fs/cgroup:/sys/fs/cgroup:ro ${DOCKER_PORT_ARGS[*]} ${DOCKER_RUN_ARGS[*]} --name $NAME $RUN_IMAGE:$VERSION $*
